...
export default function CategoriesPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    if (res.ok) { toast.success('Category added'); setNewName(''); setNewDesc(''); setShowForm(false); load(); }
  ...
  async function handleEdit(id: string) {
    ...
    if (res.ok) { toast.success('Category updated'); setEditId(null); load(); }
    else { const d = await res.json(); toast.error(d.error); }
  }

  async function handleDelete(id: string, name: string) {
    if (!confirm(`Delete category "${name}"? This may affect linked products.`)) return;
      ...
      <div className="bg-[#0e0e12] border border-[#1e1e28] rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <Tag size={32} className="text-[#222] mx-auto mb-4" />
            <p className="font-mono text-xs text-[#444]">No categories yet. Add your first one.</p>
          </div>
        ) : (
                ...
                <motion.tr key={cat.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-[#12121a] transition-colors">
                  <td className="px-5 py-4">
                    {editId === cat.id ? (
                      ...
                      <span className="font-mono text-sm font-bold text-[#e8e8f0]">{cat.name}</span>
                    )}
                  </td>
                  <td className="px-5 py-4"><span className="font-mono text-xs text-[#555]">/{cat.slug}</span></td>
                  <td className="px-5 py-4">
                    {editId === cat.id ? (
                      <input value={editDesc} onChange={(e) => setEditDesc(e.target.value)}
                        className="bg-[#12121a] border border-[#ff6b35] px-3 py-1.5 font-mono text-sm text-[#e8e8f0] focus:outline-none w-full" />
                    ) : (
                      <span className="font-mono text-xs text-[#555]">{cat.description || '—'}</span>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      {editId === cat.id ? (
                        <>
                          <button onClick={() => handleEdit(cat.id)} className="w-7 h-7 flex items-center justify-center text-emerald-400 hover:text-emerald-300 transition-colors"><Check size={14} /></button>
