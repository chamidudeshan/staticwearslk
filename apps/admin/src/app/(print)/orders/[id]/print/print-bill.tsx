    ...
    <>
      <style>{`
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page { max-width: 720px; margin: 0 auto; padding: 52px 48px; min-height: 100vh; }
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px; }
        .logo-mark { font-size: 32px; font-weight: 700; letter-spacing: -0.03em; line-height: 1; color: #e8e8f0; }
        .logo-mark span { color: #ff6b35; }
        .logo-sub { font-size: 9px; letter-spacing: 0.35em; text-transform: uppercase; color: #444; margin-top: 6px; }
        .invoice-meta { text-align: right; }
        .invoice-label { font-size: 9px; letter-spacing: 0.3em; text-transform: uppercase; color: #555; margin-bottom: 4px; }
        .invoice-id { font-size: 22px; font-weight: 700; color: #ff6b35; letter-spacing: 0.05em; }
        .invoice-date { font-size: 11px; color: #555; margin-top: 4px; }
        .rule-accent { border: none; border-top: 2px solid #ff6b35; margin: 0 0 28px; }
        .status-bar { display: flex; justify-content: space-between; align-items: center; background: #0e0e12; border: 1px solid #1e1e28; padding: 14px 20px; margin-bottom: 28px; }
        .badge { display: inline-flex; align-items: center; font-size: 9px; letter-spacing: 0.25em; text-transform: uppercase; padding: 4px 12px; border: 1px solid; }
        ...
        .badge-amber  { color: #fbbf24; border-color: rgba(251,191,36,0.2); background: rgba(251,191,36,0.08); }
        .section-label { font-size: 9px; letter-spacing: 0.35em; text-transform: uppercase; color: #ff6b35; margin-bottom: 14px; }
        .ship-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; background: #0e0e12; border: 1px solid #1e1e28; padding: 20px; margin-bottom: 28px; }
        .field-label { font-size: 9px; letter-spacing: 0.25em; text-transform: uppercase; color: #555; margin-bottom: 5px; }
        .field-value { font-size: 12px; color: #e8e8f0; line-height: 1.5; }
        .notes-block { background: #0a0a0f; border: 1px solid #1e1e28; border-top: 2px solid #ff6b35; padding: 14px 20px; margin-bottom: 28px; font-size: 12px; color: #666; font-style: italic; }
        .items-header { display: grid; grid-template-columns: 1fr 60px 80px 100px; gap: 0 16px; padding: 10px 16px; background: #0e0e12; border: 1px solid #1e1e28; border-bottom: 2px solid #ff6b35; font-size: 9px; letter-spacing: 0.25em; text-transform: uppercase; color: #555; }
        .items-header span:not(:first-child) { text-align: right; }
        .item-row { display: grid; grid-template-columns: 1fr 60px 80px 100px; gap: 0 16px; padding: 14px 16px; border: 1px solid #1e1e28; border-top: none; }
        ...
        .total-block { display: flex; justify-content: flex-end; margin-top: 2px; }
        .total-inner { background: #ff6b35; padding: 16px 24px; display: flex; align-items: center; gap: 40px; min-width: 280px; }
        .total-label { font-size: 9px; letter-spacing: 0.3em; text-transform: uppercase; color: rgba(0,0,0,0.6); flex: 1; }
        .total-value { font-size: 20px; font-weight: 700; color: #000; letter-spacing: -0.02em; }
        .footer { margin-top: 48px; padding-top: 20px; border-top: 1px solid #1e1e28; display: flex; justify-content: space-between; align-items: flex-end; }
        ...
        .print-btn { position: fixed; bottom: 28px; right: 28px; background: #ff6b35; color: #000; border: none; padding: 14px 28px; font-family: 'Courier New', monospace; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.2em; cursor: pointer; display: flex; align-items: center; gap: 8px; }
          ...
          {order.payment && (
            <div style={{ textAlign: 'right' }}>
              ...
              <span className={`badge ${order.payment.payment_status === 'paid' ? 'badge-green' : 'badge-amber'}`}>
                {order.payment.payment_status}{order.payment.payment_method ? ` · ${order.payment.payment_method}` : ''}
              </span>
            </div>
          )}
        </div>
